...
class HyperDown
{
    ...
    private function parseBlockFence(?array $block, int $key, string $line): bool
    {
        ...
        if (preg_match('/^(\s*):{3,}(.*)$/', $line, $matches)) {
            if ($this->isBlock('fence')) {
                // 如果当前已经在 fence 块中，则这是结束标记
                $this->setBlock($key)->endBlock();
            } else {
                // 否则，这是一个新的 fence 块的开始
                // 将 ::: 后面的信息字符串存起来，供 parseFence 方法使用
                $info = trim($matches[2]);
                $this->startBlock('fence', $key, $info);
            }
            return false; // 该行已处理
        } elseif ($this->isBlock('fence')) {
            // 如果当前在 fence 块中，并且不是结束标记，则将当前行设为块的一部分
            $this->setBlock($key);
            return false; // 该行已处理
        }
        return true; // 不是 fence 块，交给其他解析器处理
    }

    /**
     * 渲染 ::: 围栏块为 HTML
     * @param array $lines 块的所有行
     * @param string $info ::: 后面的信息字符串
     * @param int $start 块的起始行号
     * @return string
     */
    private function parseFence(array $lines, string $info, int $start): string
    {
        ...
        $title = '';
        $html_attrs = '';
        // New, more robust regex to parse all parts of the info string
        $pattern = '/^(?<type>\w+)(?::(?<variant>\w+))?\s*(?:\{(?<attrs>[^}]+)\})?\s*(?<title>.*)?$/';
        if (preg_match($pattern, $info, $matches)) {
            $type = $matches['type'] ?? 'info';
            $variant = $matches['variant'] ?? '';
            $attrs_str = $matches['attrs'] ?? '';
            $title = $matches['title'] ?? '';
        } elseif ($info) {
        ...
        if ($attrs_str) {
            $attr_regex = '/([\w-]+)\s*:\s*([^,]+)/';
            if (preg_match_all($attr_regex, $attrs_str, $attr_matches, PREG_SET_ORDER)) {
                foreach ($attr_matches as $match) {
                    if (count($match) === 3) {
                        $attr_key = htmlspecialchars(trim($match[1]));
                        $attr_val = htmlspecialchars(trim($match[2]));
                        $html_attrs .= " data-{$attr_key}=\"{$attr_val}\"";
                    }
                }
            }
        }

        // --- START: RAW TYPE LOGIC ---
        if ($type === 'raw') {
            $classes = "fence fence-raw";
            if ($variant) {
                $classes .= " variant-{$variant}";
            }
            $html = "<div class=\"{$classes}\"{$html_attrs}>";
            // Directly output the content without any parsing
            $html .= $content;
            $html .= '</div>';
            return $html;
        }
        ...
        if (!$final_title) { // If no title is provided
            if ($type === 'details') {
                ...
                // Default title is the type, with first letter capitalized
                $final_title = strtoupper($type);
            }
        }
        // --- END: DEFAULT TITLE LOGIC ---
        if ($type === 'details') {
            $details_attrs = '';
            if ($variant === 'open') {
                $details_attrs .= ' open';
            }
            $details_attrs .= $html_attrs;
            $html = "<details class=\"fence fence-details\"{$details_attrs}>";
            $html .= "<summary>" . $this->parseInline($final_title) . "</summary>";
            $parsedContent = $this->parse($content, false, $start + 1);
            $html .= "<div class=\"details-content\">{$parsedContent}</div>";
            $html .= '</details>';
        } else {
            ...
            $parsedContent = $this->parse($content, false, $start + 1);
            $html .= "<div class=\"fence-content\">{$parsedContent}</div>";
            $html .= '</div>';
        }
        return $html;
    }
